...
export default class StepIndicator extends PureComponent {
  ...
  render() {
    const { labels, direction, containerStyles } = this.props;
    return (
      <View
        style={[
          styles.container,
  ...
  renderStepIndicator = () => {
    let steps = [];
    const { labels, stepCount, direction, width, height } = this.props;
    ...
    return (
      <View
        onLayout={event =>
          this.setState({
            width: width > 0 ? width : event.nativeEvent.layout.width,
            height: height > 0 ? height : event.nativeEvent.layout.height
          })
        }
        style={[
          styles.stepIndicatorContainer,
          direction === "vertical"
            ? { flexDirection: "column", width: this.state.customStyles.currentStepIndicatorSize }
  ...
  renderStepLabels = () => {
    ...
    var labelViews = labels.map((label, index) => {
      const selectedStepLabelStyle = {
        ...
        color: (index === currentPosition) ? this.state.customStyles.currentStepLabelColor : this.state.customStyles.labelColor
      };
      return (
        <TouchableWithoutFeedback
